...
<<соединяется>>.
Медицинские сотрудники нуждаются в~информационных инструментах, упрощающих работу с~анатомической базой данных.
...
\begin{frame}
  ...
  \begin{enumerate}
  \item {
    Развёрнут облачный сервис с~подключением к~YandexGPT и интегрированной базой данных FMA. 
  }
  \item {
    Организован доступ к~подготовленному сервису через веб-интерфейс на~базе фреймворка ?.
  }
  \item {
    Результаты работы сервиса продемонстрированы доктору медицинских наук Шиловой Ольге Геннадьевне с~получением положительной оценки и рекомендацией для~дальнейшего развития.
    }
    \end{enumerate}
\end{frame}
